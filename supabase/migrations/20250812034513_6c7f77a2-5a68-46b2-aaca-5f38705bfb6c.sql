-- Fix cancel_bet to properly parse the response structure
...
BEGIN
  ...
  -- Parse date from the response array in match_odds_cache
  SELECT (obj->'fixture'->>'date')::timestamptz
  INTO v_fixture_date
  FROM public.match_odds_cache m,
       LATERAL jsonb_array_elements(m.data->'response') AS obj
