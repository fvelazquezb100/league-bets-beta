-- Drop existing cancel_bet function and recreate with new signature
DROP FUNCTION IF EXISTS public.cancel_bet(bigint);

-- Create new cancel_bet function to handle both single and combo bets
...
BEGIN
  ...
  IF v_bet.bet_type = 'single' THEN
    ...
    SELECT data -> v_bet.fixture_id::text INTO v_fixture_data
    FROM match_odds_cache
    WHERE id = 1;

    IF v_fixture_data IS NULL THEN
      RAISE EXCEPTION 'Fixture data not found in cache';
    END IF;

    -- Extract kickoff time
    v_kickoff_str := v_fixture_data ->> 'fixture_kickoff_time';
    IF v_kickoff_str IS NULL THEN
    ...
    -- Set earliest kickoff for single bet
  ...
  ELSIF v_bet.bet_type = 'combo' THEN
    ...
    LOOP
      -- Get fixture data from cache
      SELECT data -> v_fixture_id::text INTO v_fixture_data
      FROM match_odds_cache
      WHERE id = 1;

      IF v_fixture_data IS NOT NULL THEN
        -- Extract kickoff time
        v_kickoff_str := v_fixture_data ->> 'fixture_kickoff_time';
